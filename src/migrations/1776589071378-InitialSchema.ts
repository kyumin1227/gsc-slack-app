...
export class InitialSchema1776589071378 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."student_class_section_enum" AS ENUM('A', 'B')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."student_class_status_enum" AS ENUM('active', 'graduated')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."user_status_enum" AS ENUM('registered', 'pending_approval', 'active', 'inactive')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."user_role_enum" AS ENUM('professor', 'ta', 'class_rep', 'key_keeper', 'student')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."tag_status_enum" AS ENUM('active', 'inactive')`,
    );
    await queryRunner.query(
